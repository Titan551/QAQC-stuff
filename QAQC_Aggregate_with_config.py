...
#%%
# Set config
# Just change this one line to swap configs
config_filename = "YAML_TEST.yml"

with open(config_filename, "r") as f:
    config = yaml.safe_load(f)

# Unpack config values
folder_path = config["01_hobo_out_path"]
plots_path = config["02_plots_path"]
output_folder = config["04_processed_path"]

os_path = config["os_path"]
deployment_log_path = config["deployment_log_path"]

# Set working directory for relative paths if needed
os.chdir(os_path)

# Derived path from config
...
plot_pre_trimmed(df_files, panama_codes)
